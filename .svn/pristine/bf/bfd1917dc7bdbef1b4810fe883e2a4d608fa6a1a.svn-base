...
@Log4j
public class AdminGameDeleteController extends HttpServlet {
...
//		}
		...
		String gameSaleCount =req.getParameter("gameSaleCount"); //int 연령제한
		String isDeleted =req.getParameter("isDeleted"); //boolean 삭제여부 0 배포 1 서비스 종료
		// 불린값 정리
//		Boolean isDeleted = isDeletedParam != null ? Boolean.parseBoolean(isDeletedParam) : false;
		...
		if (gameNo != null && genreNo != null && publisherNo != null && gamePrice != null && gameAgeLimit != null) {
		    GameVO game = GameVO.builder()
		            ...
		            .gameSaleCount(Integer.valueOf(gameSaleCount))
			...
			if (gameNo == null ) {
				log.info("gameNo == null 값이 null입니다");
			}
			if (genreNo != null && publisherNo != null && gamePrice != null && gameAgeLimit != null) {
				log.info("나머지는 값이 있습니다");
			}	
		}
...
//		gameService.updateSaleCount(gameNo, saleCount)
	...
	// 1이면 서비스 종료 
	private boolean parseBoolean(String isDeleted) {
