...
//				System.out.println("퍼블리셔명 : " + publisherName);
	...
	public void updateSaleCount() {
		...
		gameList.forEach(gameVO -> {
			// 1111 ~ 7777 더할 거임
			int randomNum = random.nextInt(1111) + 6666;
			gameVO.setGameSaleCount(gameVO.getGameSaleCount() + randomNum);
	...
	// 회원의 리뷰 개수를 집계컬럼으로 저장
	public void updateReviewCount() {
		List<MemberVO> memberList = memberService.selectList();
		
		memberList.forEach(memberVO -> {
			int reviewCount = reviewService.getCountByMemberNo(memberVO.getMemberNo());
			
			int result = memberService.updateReviewCount(memberVO.getMemberNo(), reviewCount);
